...
{
    public class EnumAsUpperCamelCaseStringFormatter<T> : IYamlFormatter<T> where T : Enum
    {
        static readonly EnumMappingCache<T> Mapping;

        static EnumAsUpperCamelCaseStringFormatter()
        {
            Mapping = EnumMappingCache<T>.Create(NamingConvention.UpperCamelCase);
        }
